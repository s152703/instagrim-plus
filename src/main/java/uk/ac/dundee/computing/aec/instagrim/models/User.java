...
public class User {
    ...
    public boolean RegisterUser(String username, String Password, String email,String firstname,String lastname,String comment){
        ...
        String code="update userprofiles SET email = email + {\'"+email+"\'} where login=?";
        PreparedStatement qw = session.prepare(code);
      BoundStatement boundStatement1 = new BoundStatement(qw);
      session.execute( 
     ...
     String code1="update userprofiles SET comment = comment + {\'"+comment+"\'} where login=?";
      ...
      //session.execute("insert into userprofiles (first_name) Values('firstname') where loginusername'");
        return true;
    }
